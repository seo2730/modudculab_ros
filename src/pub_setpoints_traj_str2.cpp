...
void vector3(const geometry_msgs::Vector3Stamped::ConstPtr& vec_){ 
    vec = *vec_;   
    x = double(vec.vector.x);
    y = double(vec.vector.y);
...
int main(int argc, char **argv)
{
	...
	nh.param("pub_setpoints_traj_str2/wn", wn, 0.0);
        nh.param("pub_setpoints_traj_str2/z", z, 2.0);
	nh.param("pub_setpoints_traj_str2/x", x, 0.0);
        nh.param("pub_setpoints_traj_str2/y", y, 0.0);
	...
	//theta = wn*count*0.05;
    	...
    	pose.pose.position.x = x; //r*sin(theta);
    	pose.pose.position.y = y; //r*cos(theta);
